# 規格驅動開發 (Spec-Driven Development) 指南
...
1.  **定義 (Define)**：將模糊的需求轉化為具體的規格描述 (Input/Output/Behavior)。
2.  **合約 (Contract)**：團隊對規格達成共識，鎖定範疇 (Scope)。
3.  **實作 (Implement)**：工程師僅負責將「規格」翻譯成「程式碼」，不需在寫 code 時重新設計邏輯。
4.  **驗證 (Verify)**：測試僅針對「是否符合規格」進行驗收。
